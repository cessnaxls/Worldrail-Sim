...
function mkScene(){
 ...
 lever(throttle,'t');lever(document.getElementById('brake'),'b');
 ...
 window.WorldRailDrive={start(){driveView.classList.add('open');running=true;if(!scene)mkScene();last=performance.now();engine.runRenderLoop(()=>{tick();scene.render()});setTimeout(()=>engine.resize(),60)},stop(){running=false;driveView.classList.remove('open')}};
