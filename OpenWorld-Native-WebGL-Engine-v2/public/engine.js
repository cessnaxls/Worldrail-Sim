...
void main(){vec3 n=normalize(vN),sun=normalize(vec3(-.45,.82,.32)),v=normalize(uCam-vW);float nd=max(dot(n,sun),0.);float hemi=.30+.34*max(n.y,0.);float grain=(hash(floor(vW.xz*1.7))-.5)*.045;float spec=pow(max(dot(reflect(-sun,n),v),0.),24.)*.18;float rim=pow(1.-max(dot(v,n),0.),3.)*.055;vec3 col=uColor*(hemi+nd*.78+grain)+spec+rim;float d=distance(vW,uCam);float f=smoothstep(uFog*.52,uFog,d);vec3 fog=vec3(.55,.69,.77);col=mix(col,fog,f);col=col/(col+vec3(.28));col=pow(max(col,0.),vec3(.90));gl_FragColor=vec4(col,1.);}`;
...
// terrain is deliberately a few large meshes: cheap, solid, and prevents the "objects floating in sky" look.
add("cube",0,-1.05,0,285,1.0,285,[.20,.37,.15],0,"terrain");
// low rolling embankments beyond the town
for(let i=0;i<18;i++){let a=i/18*Math.PI*2,r=205+rng(i+70)*55;add("cube",Math.cos(a)*r,-.1,Math.sin(a)*r,24+rng(i)*28,2+rng(i+3)*6,20+rng(i+8)*32,[.18,.32,.14],a,"terrain")}
// road along Z, sidewalks/buildings
add("cube",0,-.15,0,5.5,.12,270,[.075,.082,.09],0,"road");add("cube",-7,.05,0,1,.16,270,[.45,.44,.41]);add("cube",7,.05,0,1,.16,270,[.45,.44,.41]);
...
// lane markings and roadside furniture
for(let z=-255;z<=255;z+=16)add("cube",0,.015,z,.08,.025,4.3,[.86,.83,.68],0,"marking");
for(let z=-235;z<=235;z+=34)for(let side of [-1,1]){add("cube",side*8.4,2.4,z,.08,2.4,.08,[.20,.22,.23],0,"pole");add("cube",side*8.4,4.75,z,.42,.12,.18,[.82,.80,.62],0,"lamp")}
// compact rail station / platform
add("cube",55,.38,49,28,.35,2.8,[.43,.42,.39],0,"station");add("cube",55,2.2,46.8,22,.16,3.1,[.16,.18,.20],0,"station");for(let x=36;x<=74;x+=7)add("cube",x,1.25,47,.10,1.25,.10,[.24,.26,.27],0,"station");
...
for(let i=0;i<5;i++){let tx=-150-i*9;add("cube",tx,1.55,55,4.0,1.45,1.5,[.40,.44,.47],Math.PI/2,"train");add("cube",tx,2.55,53.48,3.2,.28,.035,[.07,.12,.15],Math.PI/2,"train");add("cube",tx,2.55,56.52,3.2,.28,.035,[.07,.12,.15],Math.PI/2,"train");add("cube",tx,3.08,55,3.65,.12,1.32,[.28,.31,.33],Math.PI/2,"train")}
// parked vehicle silhouettes near town
for(let i=0;i<8;i++){let x=(i%2?10.2:-10.2),z=-150+i*38;add("cube",x,.55,z,.95,.45,1.8,[.12+.7*rng(i+44),.12+.55*rng(i+55),.15+.5*rng(i+66)],0,"car");add("cube",x,1.12,z-.15,.72,.36,.85,[.12,.16,.19],0,"car")}
