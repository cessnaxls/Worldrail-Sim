...
let f=path.normalize(path.join(base,u));if(!f.startsWith(base)){res.writeHead(403);return res.end()}
...
res.writeHead(200,{'Content-Type':t,'Cache-Control':'no-cache'});res.end(d)})}).listen(port,()=>console.log('OpenWorld engine '+port));